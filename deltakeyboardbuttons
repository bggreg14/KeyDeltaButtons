-- Delta Keyboard - Enhanced button/key controls
-- Based on the uploaded DeltaKeyboard script.
-- Custom keys now support click/hold, touch, close, minimize/restore, dragging,
-- and validation of Enum.KeyCode names.
--[[
...
G2L["13c"]["Active"] = true;
G2L["13c"]["Text"] = [[Delta Mobile - Keyboard | Made by Hat-Man]];
G2L["13c"]["Name"] = [[Title]];
...
-- Hat-Man Pro input/UI helpers
local function HM_GetServices()
	return game:GetService("UserInputService"), game:GetService("GuiService")
end

local function HM_IsMobile()
	local UIS = game:GetService("UserInputService")
	return UIS.TouchEnabled and not UIS.KeyboardEnabled
end

local function HM_KeyCode(name)
	...
	local aliases = {
		...
		["SHIFT"] = "LeftShift", ["BACKSPACE"] = "Backspace",
		["TAB"] = "Tab", ["CAPSLOCK"] = "CapsLock",
		["ALT"] = "LeftAlt", ["DELETE"] = "Delete",
		["INSERT"] = "Insert", ["HOME"] = "Home", ["END"] = "End",
		["PGUP"] = "PageUp", ["PGDN"] = "PageDown",
	}
	local key = tostring(name)
	if #key == 1 then key = string.upper(key) end
	key = aliases[string.upper(key)] or key
	return Enum.KeyCode[key]
end

local function HM_MakeCorner(parent, radius)
...
local function HM_MakeStroke(parent, thickness)
...
local function HM_AddMobileFeedback(button)
	local UIS = game:GetService("UserInputService")
	local original = button.Size
	local function down()
		button.Size = UDim2.new(original.X.Scale, original.X.Offset - 2, original.Y.Scale, original.Y.Offset - 2)
	end
	local function up()
...
local function C_56()
	...
	local IsSelecting = root.IsSelectingKey
	local ToggleShift = root.ToggleShift
	local KeysFolder = root.Keys

	local mobile = UIS.TouchEnabled and not UIS.KeyboardEnabled
	local activeInputs = {}

	local function safeKey(name)
		return HM_KeyCode(name)
	end

	local function fireDown(code)
		if not code then return end
		activeInputs[code] = true
		pcall(function() keypress(code) end)
	end

	local function fireUp(code)
		if not code then return end
		activeInputs[code] = nil
	...
	local function makeRestoreButton(key)
		...
		restore.BackgroundTransparency = 0.1
		restore.Size = UDim2.fromOffset(38, 38)
		restore.Position = key.Position
		restore.ZIndex = key.ZIndex + 2
		restore.Visible = false
		restore.Parent = KeysFolder
		HM_MakeCorner(restore, 10)
		HM_MakeStroke(restore, 1)
	...
	local function addDrag(key)
		...
		UIS.InputChanged:Connect(function(input)
			...
			key.Position = UDim2.new(startPos.X.Scale, x, startPos.Y.Scale, y)
		end)
	...
	local function createFloatingKey(keyName, label)
		local code = safeKey(keyName)
		if not code then return end

		local key = root.KEY:Clone()
		key.Parent = KeysFolder
		key.Visible = true
		key.Active = true
		key.AutoButtonColor = true
		key.Draggable = false
		key.Name = tostring(keyName)
		key.Text = label ~= "" and label or tostring(keyName)
		key:SetAttribute("KeyCodeName", tostring(keyName))
		key:SetAttribute("HatManKey", true)

		-- Make mobile targets easier to hit.
		if mobile then
			key.Size = UDim2.fromOffset(52, 52)
		end

		local close = key:FindFirstChild("Close")
		local restore = makeRestoreButton(key)
		...
		mini.Size = UDim2.fromOffset(18, 18)
		mini.Position = UDim2.new(0, -20, 0, 0)
		mini.ZIndex = key.ZIndex + 1
		mini.Parent = key
		HM_MakeCorner(mini, 6)
		...
		if mobile then
			-- Tap-only on mobile. This avoids stealing the player's movement
			-- touch for the duration of a button hold.
			key.Activated:Connect(function()
				if IsSelecting.Value then return end
				tap(code)
			end)
		else
			key.InputBegan:Connect(function(input)
		...
		addDrag(key)
		HM_AddMobileFeedback(key)
	end

	local function handleButton(button, keyName)
		local code = safeKey(keyName)
		if not code then return end

		if mobile then
			button.Activated:Connect(function()
				if IsSelecting.Value then
			...
			button.MouseButton1Up:Connect(function()
				if not IsSelecting.Value then
					fireUp(code)
				end
			end)
		end
		HM_AddMobileFeedback(button)
	end

	for _, button in ipairs(btns:GetChildren()) do
		if button:IsA("TextButton") then
			local name = button.Name
			if name == "RightShift" or name == "LeftShift" then
				handleButton(button, name)
			elseif string.sub(name, 1, 1) == "-" or string.sub(name, 1, 1) == "_" then
				handleButton(button, string.sub(name, 2))
			elseif safeKey(name) then
				handleButton(button, name)
			end
		end
	end

	-- Release simulated keys if the UI is hidden or the app loses focus.
	UIS.WindowFocusReleased:Connect(function()
		for code in pairs(activeInputs) do
			fireUp(code)
		end
...
local function C_138()
	...
	IsSelecting:GetPropertyChangedSignal("Value"):Connect(function()
		...
		elseif IsSelecting.Value == false then
			script.Parent.Title.Text = "Delta Mobile - Keyboard | Made by Hat-Man"
		end
	end)
...
task.spawn(C_138);
-- StarterGui.DeltaKeyboard.CreateKey.LocalScript
-- Improved custom-key system:
-- * Every created key is a real TextButton.
-- * Tap/click sends a key press + release.
-- * Holding the key keeps it pressed until release.
-- * Each custom key has its own minimize and close controls.
-- * Minimized keys can be restored from a small floating button.
-- * Key names are validated against Enum.KeyCode.
local function C_152()
	local script = G2L["152"]
	local root = script.Parent
	local gui = root.Parent
	local close = root:FindFirstChild("Close")
	local create = root:FindFirstChild("Create")
	local textbox = root:FindFirstChild("TextBox")
	local TweenService = game:GetService("TweenService")
	...
	local function getKeyCode(name)
		...
		-- Accept common display names as well as exact Enum.KeyCode names.
		local aliases = {
			["spacebar"] = "Space",
			["return"] = "Return",
			["enter"] = "Return",
		...
		name = aliases[lower] or name

		-- Make single letters/numbers work even when typed in lowercase.
		if #name == 1 then
	...
	local function addMinimizeButton(key)
		local mini = Instance.new("TextButton")
		mini.Name = "Minimize"
		mini.Text = "-"
		mini.TextScaled = true
		...
		mini.BackgroundColor3 = Color3.fromRGB(55,55,55)
		mini.BorderSizePixel = 0
		mini.Size = UDim2.new(0, 12, 0, 12)
		mini.Position = UDim2.new(0, -13, 0, 0)
		mini.ZIndex = key.ZIndex + 2
		mini.Parent = key
		...
		local restore = Instance.new("TextButton")
		restore.Name = "Restore"
		restore.Text = key.Text
		restore.TextScaled = true
		restore.Font = key.Font
		restore.TextColor3 = Color3.fromRGB(255,255,255)
		restore.BackgroundColor3 = Color3.fromRGB(31,31,31)
		restore.BorderSizePixel = 0
		restore.Size = UDim2.new(0, 34, 0, 34)
		restore.Position = key.Position
		restore.Visible = false
		restore.ZIndex = key.ZIndex + 1
		restore.Parent = gui
	...
	if create and textbox then
		create.Activated:Connect(function()
			local keyCode, cleanName = getKeyCode(textbox.Text)
			if not keyCode then
				textbox.Text = "Invalid KeyCode"
				return
			end

			local keysFolder = gui:FindFirstChild("Keys")
			local template = gui:FindFirstChild("KEY")
			if not keysFolder or not template then
				return
			end

			local key = template:Clone()
			key.Parent = keysFolder
			key.Visible = true
			key.Active = true
			key.AutoButtonColor = true
			key.Name = cleanName
			key.Text = cleanName
			key:SetAttribute("KeyCodeName", cleanName)

			-- The close button is kept separate so pressing it never triggers the key.
			local keyClose = key:FindFirstChild("Close")
			if keyClose then
				keyClose.ZIndex = key.ZIndex + 3
				keyClose.Activated:Connect(function()
					key:Destroy()
				end)
			end

			local _, restore = addMinimizeButton(key)
			makeDraggable(key)
			...
			if UserInputService.TouchEnabled and not UserInputService.MouseEnabled then
				-- Mobile: Activated is used as a tap, so the touch is never left
				-- holding a simulated keyboard key and movement/camera input
				-- returns immediately after the tap.
				key.Activated:Connect(function()
					if held then
						release()
					end
				...
				-- Mouse/desktop: preserve real press-and-hold behavior.
				key.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						press()
					end
