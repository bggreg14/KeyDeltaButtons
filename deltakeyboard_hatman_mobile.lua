...
local function C_56()
	...
	-- On touch-only devices, do NOT hold a simulated key from TouchStart to
	-- TouchEnd. A UI touch can otherwise make Roblox wait for the touch to
	-- finish before the mobile camera/movement controls feel responsive again.
	-- Instead, mobile uses a short tap press/release through Activated.
	...
	local function createFloatingKey(keybind, text)
		...
		key.Draggable = not touchOnly
		key.Name = keybind
		key.Text = text

		local fixedKeyCode = Enum.KeyCode[keybind]
		if not fixedKeyCode then
			key:Destroy()
			return
		end
		...
		if touchOnly then
			key.Activated:Connect(function()
				pcall(function()
					keypress(fixedKeyCode)
					keyrelease(fixedKeyCode)
				end)
			end)
		else
			key.MouseButton1Down:Connect(function()
				pcall(function() keypress(fixedKeyCode) end)
			end)
			key.MouseButton1Up:Connect(function()
				pcall(function() keyrelease(fixedKeyCode) end)
			end)
		end
	...
	local function selectOrPress(keyName, text)
		if IsSelecting.Value then
			createFloatingKey(keyName, text)
	...
	local function selectOrRelease(keyName)
		if not touchOnly and not IsSelecting.Value then
			local keyCode = Enum.KeyCode[keyName]
			if keyCode then
	...
	local function toggleShiftMode()
		if ToggleShift.Value == false then
			local shifting = script.Parent.Parent.Parent.Shifting
			shifting.Value = not shifting.Value

			for _, v in pairs(btns:GetChildren()) do
				if v:IsA("TextButton") then
					local a = v.Name
					if string.find(a, "_") then
						v.Visible = shifting.Value
					elseif string.find(a, "-") then
						v.Visible = not shifting.Value
					end
				end
			end
		else
			if IsSelecting.Value then
				IsSelecting.Value = false
				createFloatingKey("RightShift", "Shift")
			else
				pcall(function() keypress(Enum.KeyCode.RightShift) end)
				if touchOnly then
					pcall(function() keyrelease(Enum.KeyCode.RightShift) end)
				end
			end
		end
	end
	...
	for _, keys in pairs(btns:GetChildren()) do
		if keys:IsA("TextButton") then
			local keyName = keys.Name

			if keyName == "RightShift" or keyName == "LeftShift" then
				if touchOnly then
					keys.Activated:Connect(toggleShiftMode)
				else
					keys.MouseButton1Down:Connect(function()
						if ToggleShift.Value == true and not IsSelecting.Value then
					...
					keys.MouseButton1Up:Connect(function()
						if ToggleShift.Value == false then
							toggleShiftMode()
						elseif IsSelecting.Value then
							IsSelecting.Value = false
							createFloatingKey(keyName, "Shift")
						else
							pcall(function() keyrelease(Enum.KeyCode[keyName]) end)
						end
					end)
				end
			elseif string.find(keyName, "-") or string.find(keyName, "_") then
				local cleanKey = string.sub(keyName, 2)

				if touchOnly then
					keys.Activated:Connect(function()
						selectOrPress(cleanKey, keys.Text)
						-- Touch taps are intentionally press+release immediately.
						local keyCode = Enum.KeyCode[cleanKey]
						if keyCode and not IsSelecting.Value then
							pcall(function() keyrelease(keyCode) end)
						end
					end)
				else
				...
				if touchOnly then
					keys.Activated:Connect(function()
						if IsSelecting.Value then
							createFloatingKey(keyName, keys.Text)
							IsSelecting.Value = false
						else
							local keyCode = Enum.KeyCode[keyName]
							if keyCode then
					...
					keys.MouseButton1Down:Connect(function()
						selectOrPress(keyName, keys.Text)
					end)
					keys.MouseButton1Up:Connect(function()
						selectOrRelease(keyName)
					end)
				end
			end
		end
